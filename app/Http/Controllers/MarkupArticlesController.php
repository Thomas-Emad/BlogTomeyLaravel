...
class MarkupArticlesController extends Controller
{
    ...
    {
        $articles = \Auth::user()->articles()->orderByDesc('created_at')->paginate(10);
    ...
    public function markUp($user, $id)
    {
        ...
        return back()->with('message', 'This article has been added to your favorites');
    }
    public function unMark($user, $id)
    {
        MarkupArticles::where([['id_user', $user], ['id_article', $id]])->delete();
        return back()->with('message', 'This article has been deleted from your favorites');
    }
}
